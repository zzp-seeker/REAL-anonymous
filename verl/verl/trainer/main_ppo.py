...
def main(config):
    print('anon-debug ray.is_initialized:',ray.is_initialized())
    if not ray.is_initialized():
        # this is for local ray cluster
        import os
        os.environ["TOKENIZERS_PARALLELISM"] = "true"
        os.environ["NCCL_anon"] = "WARN"

        # anon-debug: for anon-debug
        import subprocess
        my_ip = subprocess.check_output(["hostname", "-i"]).decode().strip()
        print(f"anon-debug-----{my_ip}:8265")
        ...
        # ray.init(runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 'NCCL_anon': 'WARN'}})
